...
from Project_Template.custom_exception import InvalidURLException
from Project_Template.logger import logger
    ...
    def _verify_vid_id_len(vid_id, __expected_len=11):
        ...
        if len_of_vid_id != __expected_len:
            raise InvalidURLException(
                f"Invalid video with length: {len_of_vid_id}, expected: {__expected_len}"
            )
...
def render_Youtube_video(URL: str, width: int=780, height: int=600) -> str:
    try:
        ...
        data = Data(URL).data()
        if data["publishdata"]is not None:
            time = get_time_info(URL)
            video_ID = data["id"]
            embed_URL = f"http://www.youtube.com/embed/{video_ID}?start={time}"
